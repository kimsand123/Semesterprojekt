...
def login(request):
    ...
    try:
        ...
        game_service_ip, game_service_port, player_id = register_user_with_game_service(AUTH_SERVICE_ACCESS_KEY,
                                                                                        user_token,
                                                                                        user_object)

        # If gameservice responds with proper data,
        # return the userobject, usertoken and gameservice ip and port to client.
        if game_service_ip is not None and player_id is not None:
            user_object['user_token'] = user_token
            user_object['gameservice_ip'] = game_service_ip
            user_object['gameservice_port'] = game_service_port
            user_object['player_id'] = player_id
            print(
...
def register_user_with_game_service(service_key, user_token, user_object):
    ...
    try:
        ...
        game_service_ip = data["gameservice_ip"]
        game_service_port = data['gameservice_port']
        ...
        return game_service_ip, game_service_port, player_id
